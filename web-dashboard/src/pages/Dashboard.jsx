...
export default function Dashboard() {
  ...
  const activityRows = useMemo(() => {
    ...
    return apiData.loads.map((load) => ({ id: load._id, area: load.routeId?.barangay || 'Unassigned area', driver: load.staffId?.name || 'Staff member', length: load.length ? (load.length / 100).toFixed(1) : '—', width: load.width ? (load.width / 100).toFixed(1) : '—', height: load.height ? (load.height / 100).toFixed(1) : '—', slope: '—', tonnage: formatTonnes(load.tonnesEstimate), time: new Date(load.arrivedAt).toLocaleTimeString('en-PH', { hour: 'numeric', minute: '2-digit' }) }));
  }, [apiData.loads]);
